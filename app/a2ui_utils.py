...
def _extract_a2ui_messages(text: str) -> list[dict]:
    ...
    for value in values:
        if not isinstance(value, dict):
            continue
        # Support direct component tree: [{"Card": {"child": {"Column": {"children": [...]}}}}]
        if "Card" in value and not any(k in value for k in _A2UI_KEYS):
            card_obj = value["Card"]
            col_obj = card_obj.get("child", {}).get("Column", {})
            children_list = col_obj.get("children", [])
            comps = []
            cids = []
            for idx, ch in enumerate(children_list):
                cid = f"comp_{idx}"
                cids.append(cid)
                comps.append({"id": cid, "component": ch})
            comps.append({
                "id": "main_col",
                "component": {"Column": {"children": {"explicitList": cids}}}
            })
            comps.append({
                "id": "root_card",
            ...
            messages.extend([
                {"beginRendering": {"surfaceId": "default", "root": "root_card"}},
                {"surfaceUpdate": {"surfaceId": "default", "components": comps}}
            ])
            continue
...
def a2ui_callback(
    ...
    for part in llm_response.content.parts:
        ...
        if not any(k in text for k in _A2UI_KEYS) and "Card" not in text:
            continue
        ...
        # In ADK web dev UI, the angular frontend parses <a2ui-json>...</a2ui-json> directly from text
        # and displays the app-a2ui-canvas component. We also provide clear markdown summary text
        # so if the dev UI canvas fails or is flaky, the user immediately sees a clean report instead of blank.
        a2ui_tag_text = f"<a2ui-json>\n{json.dumps(messages, indent=2)}\n</a2ui-json>"
        summary_lines = []
        for m in messages:
            su = m.get("surfaceUpdate", {})
            for c in su.get("components", []):
                ...
                if "Text" in comp:
                    ...
                    lit = t_val.get("literalString") if isinstance(t_val, dict) else str(t_val)
                    hint = comp["Text"].get("usageHint", "")
                    if hint in ("h1", "title"):
                        summary_lines.append(f"### {lit}")
                    elif hint in ("h2", "subheading"):
                        summary_lines.append(f"**{lit}**")
                    else:
                        summary_lines.append(lit)
                elif "Image" in comp:
                    img_url = comp["Image"].get("url", {})
                    url_str = img_url.get("literalString") if isinstance(img_url, dict) else str(img_url)
                    summary_lines.append(f"![Screenshot]({url_str})")

        human_summary = "\n\n".join(summary_lines)
        combined_text_part = types.Part(text=f"{a2ui_tag_text}\n\n{human_summary}")
        new_parts.append(combined_text_part)
